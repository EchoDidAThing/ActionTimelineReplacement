...
#region Main
public class ActionCastTimelineMain
{
    const string type = "ActionCastTimeline";
    const string typename = "Action Cast Timeline";
    const string typenameplural = "Action Cast Timeliness";
    public static void Draw(string mainkey, ref Configuration.ReplacementSet _activeSet, ref string search) {

        Dictionary<uint, ActionCastTimelineConfig> Writer = _activeSet.ActionCastTimelineWriter;
        var GetName = ActionCastTimelineManager.GetName;
        var CreateEntry = ActionCastTimelineConfig.CreateEntry;
        ...
        if (subList) {
            ...
            foreach (var key in Writer.Keys) {
                if (ImGui.CollapsingHeader($"#{key:D5} - " + GetName(key))) {
                    ...
                    var old = ActionCastTimelineManager.GetOriginal(key);
                    ...
                    #region Datainputs
                    UiGlobals.DrawUShort("Action Timeline", type, key, enablechanges, ref LocalWriter.Replacement.ActionTimeline, old.ActionTimeline, true, ["Action_Path"]);
                    UiGlobals.DrawUShort("VFX", type, key, enablechanges, ref LocalWriter.Replacement.CastVfx, old.CastVfx, true, ["VFX_Path"]);
                    UiGlobals.DrawItemSeparator();
                    continue;
