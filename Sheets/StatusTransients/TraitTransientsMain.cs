...
#region Main
public class StatusTransientsMain
{
    const string type = "StatusTransients";
    const string typename = "Status Transient";
    const string typenameplural = "Status Transients";
    ...
    public static void Draw(string mainkey, ref Configuration.ReplacementSet _activeSet, ref string search) {
        ...
        Dictionary<uint, StatusTransientsConfig> Writer = _activeSet.StatusTransientsWriter;
        var GetName = StatusTransientsManager.GetName;
        var CreateEntry = StatusTransientsConfig.CreateEntry;
        ...
        if (subList) {
            ...
            {
                var old = StatusTransientsManager.GetOriginal(key);
                if (ImGui.CollapsingHeader($"#{key:D5} - " + GetName(key))) {
                    ...
                    #region Datainputs
                    //supressing icon for now, renabel when fixed
